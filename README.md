...
| 🎁 | **🆕 连签档位兑换** — 自动兑换连签 7 / 14 / 28 天档位奖励（14 天 50 积分、28 天 150 积分） |
...
成长中心的连续签到有 7 / 14 / 28 天三档奖励，**必须手动点兑换才会发放**，过期不补：
...
脚本每次运行都会读 `POST /v2/activity/growth/streak` 的 `redemption_status`，**只在档位状态为"已解锁且未领过"时才兑换**，`locked` / `claimed` 一律跳过；兑换失败只写进汇报、不影响已完成的签到。

> **⚠️ 关于端点：已定位，payload 为推断值**
>
> 兑换端点 `POST /v2/activity/growth/redeem` 是**通过实际探测确认存在**的（非 404，返回结构化错误）。请求体 `{"tier": <天数 int>}` 的依据是：
> - `tier` 传字符串 → `invalid redemption request`（JSON 绑定失败，说明字段是整数）
...
- `StartInterval`：每 **30 分钟**自查一次；Mac 醒着时即触发。配合脚本幂等，保证"只要当天开过机 / 唤过醒，就会签到"——这是实现"早上开盖自动签"的关键，因为 `RunAtLoad` **只在开机/登录时跑，纯睡眠→唤醒不会触发它**。
- `RunAtLoad`：每次开机 / 登录立即跑一次 = **补领**。
- **沉睡补领（有限）**：若 Mac 在 21:30/22:30 处于睡眠，launchd 在唤醒后补跑**那几个错过的点**；但"早上开盖"时今天的时间点还在未来、没有可补的点，所以不会因唤醒而签到——这正由上面的 `StartInterval` 兜底。
- **不覆盖的情况**：整天空机且当天完全不开机，那天会漏签；但只要当天任意一次开机/登录或唤醒后保持醒着 ≥30 分钟，`StartInterval` 必跑一次兜底。
